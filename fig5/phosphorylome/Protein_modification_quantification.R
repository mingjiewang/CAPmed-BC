...
gene.exp = data.frame(
  ...
  Expression = unlist(datExp[rownames(datExp) == site.want, 1:6]),
...
p1 <- ggviolin(gene.exp,x = "Group",y="Expression",color = NA,
  ...
  annotate("text",x=1.5,y=max(gene.exp$Expression)*1.3,label=sprintf("%0.4f",datExp$P.value[rownames(datExp) == site.want]))
p1
save_plot("Protein.site.modification.violinplot.pdf",p1,base_width = 5, base_height = 6)
