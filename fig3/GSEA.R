...
# match gene
gene.id.enrich <- database[match(diff$GeneName, names(database))]
gene.id.enrich <- data.frame(GeneName = names(gene.id.enrich), ID = as.character(gene.id.enrich))
deg.file.enroll <- merge(gene.id.enrich, diff, by.x = "GeneName", by.y = "GeneName")
...
## output table
edo.gsea.out = as.data.frame(edo.gsea)[1:100,]
